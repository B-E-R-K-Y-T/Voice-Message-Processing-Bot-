...
from config import API_TOKEN, file_schedule_tasks, stop_app_lock, StopApp, MAX_PROCESS_FILES, files_queue, bot_lock, \
    MAX_FILE_SIZE
...
def handle_voice(message: Message):
    ...
    bot.reply_to(message=message,  text='Ваш файл взят в работу! Пожалуйста, подождите...')
...
def file_converter():
    while True:
        ...
        for _ in range(MAX_PROCESS_FILES):
            ...
            logging.info(f'Starting to download file: {file.file_path} for user: {message.from_user.id}')
